...
class GCLSTM(torch.nn.Module):
    ...
    def _calculate_input_gate(self, X, edge_index, edge_weight, H, C, h_agg):
        I = torch.matmul(X, self.W_i)
        I = I + self.conv_i(H, edge_index, edge_weight, h_agg)
        I = I + self.b_i
        I = torch.sigmoid(I)
        return I

    def _calculate_forget_gate(self, X, edge_index, edge_weight, H, C, h_agg):
        F = torch.matmul(X, self.W_f)
        F = F + self.conv_f(H, edge_index, edge_weight, h_agg)
        F = F + self.b_f
        F = torch.sigmoid(F)
        return F

    def _calculate_cell_state(self, X, edge_index, edge_weight, H, C, I, F, h_agg):
        T = torch.matmul(X, self.W_c)
        T = T + self.conv_c(H, edge_index, edge_weight, h_agg)
        T = T + self.b_c
        T = torch.tanh(T)
        C = F * C + I * T
        return C

    def _calculate_output_gate(self, X, edge_index, edge_weight, H, C, h_agg):
        O = torch.matmul(X, self.W_o)
        O = O + self.conv_o(H, edge_index, edge_weight, h_agg)
        O = O + self.b_o
        O = torch.sigmoid(O)
        return O
    ...
    def forward(
        ...
        I = self._calculate_input_gate(X, edge_index, edge_weight, H, C, h_agg)
        F = self._calculate_forget_gate(X, edge_index, edge_weight, H, C, h_agg)
        C = self._calculate_cell_state(X, edge_index, edge_weight, H, C, I, F, h_agg)
        O = self._calculate_output_gate(X, edge_index, edge_weight, H, C, h_agg)
